...
.PHONY: help dev web api api-build api-stop search search-build search-stop dev-network check-dev

help:
	@echo "Targets:"
	@echo "  make dev           Start web, api, and search together"
	@echo "  make web           Start the Vite web app on port $(WEB_PORT)"
	@echo "  make api           Start the API Docker container on port $(API_PORT)"
